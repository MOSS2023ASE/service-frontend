...
export async function readTutorXlsx(file) {
  const XLSX = require('xlsx');
  const dataBinary = await readAsBlob(file);
  const workBook = XLSX.read(dataBinary, { type: 'binary', cellDates: true });
  const Sheet1 = workBook.Sheets.Sheet1;
  ...
  let data = new Map();
  while ('A' + i.toString() in Sheet1) {
    let student_id = Sheet1[colMap.get('辅导师学号') + i.toString()].v.toString();
    let subject_id = Sheet1[colMap.get('辅导科目') + i.toString()].v;
    if (!data.has(student_id)) {
      data.set(student_id, []);
    }
    data.get(student_id).push(subject_id);
    i++;
  }
  return data;
}